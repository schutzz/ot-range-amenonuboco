...
def poll_once() -> int:
    ...
    query = {
        ...
        #
        # 【Phase9決定事項#139で降順へ訂正】昇順(挿入順そのまま)のまま
        # size:50固定だと、総ドキュメント数が50を超えた瞬間から**常に
        # 最古の50件だけを見続け、新着ドキュメントに永久に到達できなく
        # なる**バグだった。旧来はDNP3トラフィックが攻撃時のみ数件しか
        # 流れず、総数が50を超えることが無かったため露呈しなかった。
        # Phase9でcc_scada_masterが実DNP3の周期Pollを流すようになり、
        # 総数が50を素早く超えるようになって発覚した(実機確認: 総数136件、
        # 昇順size:50で取得すると全て運用開始直後の古いドキュメントのみ、
        # 攻撃パケットが1件も視界に入らずALERTが1件も発火しなかった)。
        # 降順にすることで毎回「直近に挿入された50件」を見るようになり、
        # 新着が確実に視界へ入る(1プロトコルにつき単一のstructurerしか
        # 書き込まない前提=決定事項#39のため、_doc降順は挿入順の逆順として
        # 安定して機能する)。
        "sort": [{"_doc": "desc"}],
